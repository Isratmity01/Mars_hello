...
*/
public class StickersRecentsManager extends ArrayList<Emojicon> {
    ...
    private static StickersRecentsManager sInstance;

    private Context mContext;

    private StickersRecentsManager(Context context) {
        mContext = context.getApplicationContext();
        loadRecents();
    }

    public static StickersRecentsManager getInstance(Context context) {
        if (sInstance == null) {
            synchronized (LOCK) {
                if (sInstance == null) {
                    sInstance = new StickersRecentsManager(context);
                }
            }
        }
        return sInstance;
