...
import github.ankushsachdeva.emojicon.Stickers.NatureStickers;
...
public class EmojiconsPopup extends PopupWindow implements ViewPager.OnPageChangeListener, EmojiconRecents,StickerRecents {
	private int mEmojiTabLastSelectedIndex = -1;
	private View[] mEmojiTabs;
    private View[] mStickerTabs;
	private Button[] mTopTabs;
	private PagerAdapter mEmojisAdapter;
	private PagerAdapter mStickerAdapter;
    private HorizontalScrollView stickerTab;
	private HorizontalScrollView emoticonstab;
	private EmojiconRecentsManager mRecentsManager;
	private StickersRecentsManager mRecentsManagerst;
	private int keyBoardHeight = 0;
	int flag=0; //0 means emoji selected,1 means stickers selected
	private Boolean pendingOpen = false;
	private Boolean isOpened = false;
	OnEmojiconClickedListener onEmojiconClickedListener;
	StickerGridView.OnStickerClickedListener onStickerClickedListener;
	...
	/**
	 * Constructor
	 * @param rootView	The top most layout in your view hierarchy.
     *                  The difference of this view and the screen height will be used to calculate the keyboard height.
	...
	public void setEmojiClickListener(StickerGridView.OnStickerClickedListener listener){
	...
	private View createCustomView() {
		...
		emoticonstab = (HorizontalScrollView) view.findViewById(R.id.emojis_tab);
		emojisPager = (ViewPager) view.findViewById(R.id.emojis_pager);
		stickerTab = (HorizontalScrollView) view.findViewById(R.id.stickers_tab);
		emojisPager.setOnPageChangeListener(this);
		EmojiconRecents recents = this;
		StickerRecents recentstickers = this;
		...
		mStickerAdapter = new StickersPagerAdapter(
				Arrays.asList(
		...
		mTopTabs[1] = (Button)view.findViewById(R.id.stickers);

		mTopTabs[0].setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				flag = 0;
				mTopTabs[0].setBackgroundResource(R.drawable.ic_group_4_selected);
				mTopTabs[0].setTextColor(Color.parseColor("#ffffff"));
				mTopTabs[1].setBackgroundResource(R.drawable.ic_group_5);
				mTopTabs[1].setTextColor(Color.parseColor("#26beff"));
				emoticonstab.setVisibility(View.VISIBLE);
				stickerTab.setVisibility(View.GONE);
				onPageSelected(0);
				emojisPager.setAdapter(mEmojisAdapter);

				emojisPager.getAdapter().notifyDataSetChanged();

			}
		});
		mTopTabs[1].setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				flag = 1;
				mTopTabs[0].setBackgroundResource(R.drawable.ic_group_4);
				mTopTabs[0].setTextColor(Color.parseColor("#26beff"));
				mTopTabs[1].setBackgroundResource(R.drawable.ic_group_5_selected);
				mTopTabs[1].setTextColor(Color.parseColor("#ffffff"));
				emoticonstab.setVisibility(View.GONE);
				stickerTab.setVisibility(View.VISIBLE);
				emojisPager.setAdapter(mStickerAdapter);
				emojisPager.getAdapter().notifyDataSetChanged();
			}
		});
		//stickers
		...
		if (page == 0) {
			...
			mRecentsManagerst = StickersRecentsManager.getInstance(view.getContext());
	...
	public void addRecentSticker(Context context, Emojicon emojicon) {
		StickerGridView fragment = ((StickersPagerAdapter)emojisPager.getAdapter()).getRecentFragment();
		//fragment.addRecentSticker(context, emojicon);
	}
	...
	private static class StickersPagerAdapter extends PagerAdapter {
		private List<StickerGridView> views;

		public StickersRecentsGridView getRecentFragment(){
			for (StickerGridView it : views) {
				if(it instanceof StickersRecentsGridView)
					return (StickersRecentsGridView)it;
			}
			return null;
		}
		public StickersPagerAdapter(List<StickerGridView> views) {
			super();
			this.views = views;
		}
